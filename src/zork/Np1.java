		...
		loop: for(i = 0; i < i__1; i += 2)
		{
			...
			in_do_while: do
			{
				switch (GOTO)
				{
					...
					case 75:
						if (np.parse.pv_1.act != 0
								&& (np.parse.vvoc[np.parse.pv_1.act - 1] != r50wal || prep != 0))
						{
							GOTO = 200;
							continue in_do_while;
						...
						{
							/* !THEN CHK FOR DIR. */
							if (lbuf1 == np.parse.dirvoc_1.dvoc[j - 1]
									&& lbuf2 == np.parse.dirvoc_1.dvoc[j])
							{
								GOTO = 2000;
								continue in_do_while;
							}
					...
					case 200:
						...
						for (j = 1; j <= i__2; j += 3)
						{
							...
							if (lbuf1 == np.parse.prpvoc_1.pvoc[j - 1]
									&& lbuf2 == np.parse.prpvoc_1.pvoc[j])
							{
								GOTO = 4000;
								continue in_do_while;
							}
					...
					case 6100:
						...
						if (vbflag)
						{
							game.dsub.rspsub_(620,
									vars.objcts_1.odesc2[vars.advs_1.avehic[vars.play_1.winner
											- 1] - 1]);
						}
						return ret_val;
		...
		loop2: do
		{
			switch (GOTO)
			{
				case 1500:
					...
					if (vars.orphs_1.oflag != 0 && vars.orphs_1.oprep != 0 && prep == 0
							&& np.parse.pv_1.o1() != 0 && np.parse.pv_1.o2() == 0
							&& np.parse.pv_1.act == vars.orphs_1.oact)
					{
						{
							GOTO = 11000;
							continue loop2;
